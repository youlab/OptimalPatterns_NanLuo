...
h1 = tf.layers.dense(h0, 256, kernel_initializer=he_init, activation=lrelu, name="fcl2")
#h1_drop=tf.layers.dropout(h1, dropout_rate, training=training)
h2 = tf.layers.dense(h1, 128, kernel_initializer=he_init, activation=lrelu, name="fcl3")
#h2_drop=tf.layers.dropout(h2, dropout_rate, training=training)
h3 = tf.layers.dense(h2, 64, kernel_initializer=he_init, activation=lrelu, name="fcl4")
...
with tf.Session() as sess:
    saver.restore(sess, "NN4/saved_model/mymodel.ckpt")
    loss_train4 = d_loss.eval(feed_dict={in_array:train_input, out: train_output})
    loss_test4 = d_loss.eval(feed_dict={in_array:test_input, out: test_output})  
    pred_train4=logits.eval(feed_dict={in_array:train_input, out: train_output})
    pred_test4=logits.eval(feed_dict={in_array:test_input, out: test_output}) 

with tf.Session() as sess:
    saver.restore(sess, "NN5/saved_model/mymodel.ckpt")
    loss_train5=d_loss.eval(feed_dict={in_array:train_input, out: train_output})
    loss_test5=d_loss.eval(feed_dict={in_array:test_input, out: test_output})  
    pred_train5=logits.eval(feed_dict={in_array:train_input, out: train_output})
    pred_test5=logits.eval(feed_dict={in_array:test_input, out: test_output}) 

with tf.Session() as sess:
    saver.restore(sess, "NN6/saved_model/mymodel.ckpt")
    loss_train6 = d_loss.eval(feed_dict={in_array:train_input, out: train_output})
    loss_test6 = d_loss.eval(feed_dict={in_array:test_input, out: test_output})  
    pred_train6=logits.eval(feed_dict={in_array:train_input, out: train_output})
    pred_test6=logits.eval(feed_dict={in_array:test_input, out: test_output}) 
    
with tf.Session() as sess:
    saver.restore(sess, "NN7/saved_model/mymodel.ckpt")
    loss_train7 = d_loss.eval(feed_dict={in_array:train_input, out: train_output})
    loss_test7 = d_loss.eval(feed_dict={in_array:test_input, out: test_output})  
    pred_train7=logits.eval(feed_dict={in_array:train_input, out: train_output})
    pred_test7=logits.eval(feed_dict={in_array:test_input, out: test_output}) 
    #pdb.set_trace()
saved_data_train=np.zeros((len(pred_train1),12)) #gt, pred1X7,indx, pred_ensemble, mdiv_train, error_train
saved_data_test=np.zeros((len(pred_test1),12)) #gt, pred1,pred2,pred3,indx, pred_ensemble, mdiv_train, error_train

for kb in range(train_size):
    pred_train=np.r_[pred_train1[kb],pred_train2[kb],pred_train3[kb],pred_train4[kb],pred_train5[kb],pred_train6[kb],pred_train7[kb]]
    rmse1=rmse(pred_train[0]*np.ones((6)),np.delete(pred_train,0))
    rmse2=rmse(pred_train[1]*np.ones((6)),np.delete(pred_train,1))
    rmse3=rmse(pred_train[2]*np.ones((6)),np.delete(pred_train,2))
    rmse4=rmse(pred_train[3]*np.ones((6)),np.delete(pred_train,3))
    rmse5=rmse(pred_train[4]*np.ones((6)),np.delete(pred_train,4))
    rmse6=rmse(pred_train[5]*np.ones((6)),np.delete(pred_train,5))
    rmse7=rmse(pred_train[6]*np.ones((6)),np.delete(pred_train,6))
    m_train=np.r_[rmse1,rmse2,rmse3,rmse4,rmse5,rmse6,rmse7]
...
for kb in range(test_size):  
    pred_test=np.r_[pred_test1[kb],pred_test2[kb],pred_test3[kb],pred_test4[kb],pred_test5[kb],pred_test6[kb],pred_test7[kb]]
    rmse1=rmse(pred_test[0]*np.ones((6)),np.delete(pred_test,0))
    rmse2=rmse(pred_test[1]*np.ones((6)),np.delete(pred_test,1))
    rmse3=rmse(pred_test[2]*np.ones((6)),np.delete(pred_test,2))
    rmse4=rmse(pred_test[3]*np.ones((6)),np.delete(pred_test,3))
    rmse5=rmse(pred_test[4]*np.ones((6)),np.delete(pred_test,4))
    rmse6=rmse(pred_test[5]*np.ones((6)),np.delete(pred_test,5))
    rmse7=rmse(pred_test[6]*np.ones((6)),np.delete(pred_test,6))
    m_test=np.r_[rmse1,rmse2,rmse3,rmse4,rmse5,rmse6,rmse7]
...
loss_train_ensemble= (np.square(saved_data_train[:,0]-saved_data_train[:,9])).mean(axis=None)
loss_test_ensemble= (np.square(saved_data_test[:,0]-saved_data_test[:,9])).mean(axis=None)
